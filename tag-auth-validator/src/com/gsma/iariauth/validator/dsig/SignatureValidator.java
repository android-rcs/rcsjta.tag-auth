...
/**
 * A class that performs validation of a signature in an
 * IARI Authorization document
 */
public class SignatureValidator {
	 ...
	 */
	public SignatureValidator(Document doc, Element signatureNode, Set<String> expectedRefs, AuthType authType) {
	 ...
	 * STATUS_VALID: signature validation and verification were both completed successfully
	 * STATUS_INVALID: signature validation failed
	 * STATUS_REVOKED: signature verification failed as a result of there being a revoked certificate
	 * STATUS_UNSIGNED: signature validation succeeded, but verification failed because it was not possible
	 *                  to build a path (such as expired certificate, no suitable trust anchor, etc)
	 */
	public int validate() {
		...
		if(result == ProcessingResult.STATUS_OK) {
			...
			if(result == ProcessingResult.STATUS_VALID_HAS_ANCHOR) {
				signatureInfo = SignatureInfo.create(
					authType,
					signatureId,
					validationContext.getRootCert(),
					validationContext.getEntityCert(),
					validationContext.getCertificatePath(),
					validationContext.getPolicies()
				);	
			}
		}
	...
	private int validateXMLSignature() {
		...
		/*
		 * ASSERTION
		 * The implementation MUST ensure that each ds:Signature property
		 * required by this specification meets the syntax requirements of
		 * [XMLDSIG-Properties].
		 */
		...
		try {
			...
			error = new Artifact("Unexpected exception processing signature: " + e.getLocalizedMessage());
		...
		/*
		 * ASSERTION
		 * A validator validates a signature according to Core Validation
		 * including Reference Validation and Signature Validation, all of which
		 * are defined in [XMLDSIG11].
		 *
		 * ASSERTION
		 * If a ds:KeyInfo element is present, then a validator MUST check
		 * that the ds:KeyInfo conforms to the [XMLDSIG11] specification by
		 * performing Basic Path Validation [RFC5280] on the signing key. A
		 * validator SHOULD perform revocation checking as appropriate.
		 * 
		 * ASSERTION
		...
		try {
			...
			error = new Artifact("Unexpected exception checking signature validity: " + e.getLocalizedMessage());
			return (status = error.status);
		}		 
		...
		/*
		 * ASSERTION
		 * The implementation MUST ensure that the ds:SignatureMethod algorithm
		 * used in the ds:SignatureValue element is one of the signature
		 * algorithms. The ds:Signature MUST be produced using a key of the
		 * recommended key length or stronger (meaning larger than 2048 bits).
		 */
	...
	private int validateSignatureProperties() {
		...
		/*
		 * ASSERTION
		 * The implementation MUST ensure that the Algorithm attribute of the
		 * ds:CanonicalizationMethod element is one of the canonicalization
		 * algorithms.
		 */
		...
		/*
		 * ASSERTION
		 * The implementation MUST ensure that this ds:Object element contains
		 * a single ds:SignatureProperties element that contains a different
		 * ds:SignatureProperty element for each property required by this
		 * specification.
		 *
		 * ASSERTION
		 * The implementation MUST ensure that a signature includes a
		 * ds:Object element within the ds:Signature element. This ds:Object
		 * element MUST have an Id attribute that is referenced by a ds:Reference
		 * element within the signature ds:SignedInfo element.
		 */
				...
				if(contentOb instanceof DOMStructure) {
					...
					if(nsURI != null && nsURI.equals(SignatureConstants.SIG_PROPERTY_NS)) {
						...
						if(nodeName.equals(SignatureConstants.SIG_PROPERTY_PROFILE_NAME)) {
							Node URINode = propNode.getAttributes().getNamedItem(SignatureConstants.SIG_PROPERTY_URI_NAME);
							if(URINode != null) {
								profileCount++;
		...
		/*
		 * ASSERTION
		 * If a validator finds that a file entry matching the naming
		 * convention for a distributor signature that does not contain
		 * a dsp:Role signature property having the URI for a distributor
		 * role, then the validator MUST treat the signature as being in error.
		 * 
		 * ASSERTION
		 * If a validator finds that a file entry matching the naming convention
		 * for an author signature that does not contain a dsp:Role signature
		 * property having the URI for a author role, then the validator MUST
		 * treat the signature as being in error.
		 */
		...
		/*
		 * ASSERTION
		 * Tests that each signature contains a dsp:Profile signature
		 * properties element with the correct URI attribute value.
		 */
		if(profileCount != 1 || profileURI == null || !profileURI.equals(SignatureConstants.SIG_PROPERTY_PROFILE_URI)) {
		...
		/*
		 * ASSERTION
		 * Tests that each signature contains a dsp:Identifier signature
		 * properties element compliant with XMLDSIG-Properties. To pass a
		 * validator must fail to validate this document if there are two
		 * dsp:Identifier elements.
		 */
		...
		/*
		 * ASSERTION
		 * A validator MUST support the RSAwithSHA256 and DSAwithSHA1 signature
		 * algorithms.
		 * 
		 * ASSERTION
		 * An implementation SHOULD support the ECDSAwithSHA256 signature
		 * algorithm.
		 * 
		 * ASSERTION
		 * The implementation MUST ensure that the ds:SignatureMethod algorithm
		...
		/*
		 * ASSERTION
		 * If one or more certificates are in the ds:KeyInfo element, the
		 * implementation MUST ensure that they are of the mandatory certificate
		 * format.
		 */
		...
		while (keyInfoItems.hasNext()) {
			...
			if (item instanceof PGPData
					|| item instanceof SPKIData
					|| item instanceof MgmtData) {
	...
	private int validateReferences() {
		...
		/*
		 * ASSERTION
		 * Tests if every ds:Reference used within a signature has a
		 * URI attribute.
		 *
		 * ASSERTION
		 * Tests if every ds:Reference used within a signature is a
		 * reference to known element in the enclosing XML document.
		 */
		 ...
		 * The implementation MUST ensure that a signature includes a
		 * ds:Object element within the ds:Signature element. This ds:Object
		 * element MUST have an xml:id attribute that is referenced by a ds:Reference
		 * element within the signature ds:SignedInfo element.
		 */
		...
		/*
		 * ASSERTION
		 * The implementation MUST ensure that a signature includes references to
		 * each of the required members of the containing iari-authorization element.
		 */
	...
	private boolean validateReference(String id, Reference ref) {
		 ...
		 * 
		 * ASSERTION
		 * The implementation MUST ensure that a ds:Reference to same-document
		 * XML content has a ds:Transform element child that specifies the
		 * canonicalization method. Canonical XML 1.1 MUST be specified as the
		 * Canonicalization Algorithm for this transform. A ds:Reference that
		 * is not to same-document XML content MUST NOT have any ds:Transform
		 * elements.
		 */
		...
		public Data dereference(URIReference uriReference, XMLCryptoContext context) throws URIReferenceException {
			...
			if(refURI.charAt(0) == '#') {
				...
				result = new ApacheData() {
					public XMLSignatureInput getXMLSignatureInput() { return new XMLSignatureInput(refNode); }
				};
			}
			return result;
		}
